pipeline {
  ...
  parameters {
    choice(name: 'STAGE', choices: ['hml', 'prd'], description: 'Ambiente de deploy (hml/prd)')
  }
  ...
  environment {
    ...
    // Repositórios (ajuste APP_REPO_URL se a app estiver em outro repo)
    APP_REPO_URL          = 'https://github.com/wallafidevops/desafio-jenkins-kustomize.git'         // app/mono-repo
    GITOPS_REPO_URL       = 'https://github.com/wallafidevops/desafio-jenkins-kustomize.git'         // gitops
    ...
    // Artefatos S3 (mantenha único ou separe por ambiente no Setup vars)
    ARTIFACTS_BUCKET_PATH = 'wordpress-216989136189'

    // Git (para commits no GitOps)
  ...
  stages {
    ...
    stage('Setup vars (stage/branches)') {
      steps {
        script {
          // 1) Resolve STAGE: param > branch (main => prd, senão hml)
          env.STAGE = params.STAGE ?: ((env.BRANCH_NAME == 'main') ? 'prd' : 'hml')

          // 2) Branches por ambiente (ajuste se necessário)
          env.GITOPS_BRANCH = (env.STAGE == 'prd') ? 'main'    : 'develop'
          env.APP_BRANCH    = (env.STAGE == 'prd') ? 'main'    : 'develop'

          // 3) Prefixar ambiente nas variáveis
          ...
          env.SONAR_CRED_ID    = "${env.STAGE}-sonar-token"       // ex.: hml-sonar-token / prd-sonar-token

          echo "STAGE=${env.STAGE}"
          echo "APP_BRANCH=${env.APP_BRANCH} | GITOPS_BRANCH=${env.GITOPS_BRANCH}"
      ...
      steps {
        withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', credentialsId: 'aws']]) {
          sh '''
            ...
            echo "Trivy scan -> $AWS_ECR_URI/$IMAGE_NAME:$FULL_SHA"
            ...
            if [ -f "./reports/gl-container-scanning-report.json" ]; then
              aws s3 cp ./reports/gl-container-scanning-report.json "s3://$ARTIFACTS_BUCKET_PATH/trivy/gl-container-scanning-report-$(date +%F-%T).json"
            else
              echo "Relatório Trivy não encontrado."
              exit 1
            fi
          '''
        }
    ...
    stage('SonarQube Analysis') {
      steps {
        script {
          def sonarCred = env.SONAR_CRED_ID
      ...
      steps {
        withCredentials([ usernamePassword(credentialsId: 'git', usernameVariable: 'GITHUB_USERNAME', passwordVariable: 'GITHUB_PASSWORD') ]) {
          sh '''
            ...
            cd "$OVERLAY_DIR"
            # Troca placeholder -> imagem/tag correta (IMAGE_NAME já tem prefixo do ambiente)
